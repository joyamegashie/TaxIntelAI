...
class BusinessDetector:
    ...
    def _load_models(self):
        """Load pre-trained models if they exist"""
        try:
            classifier_path = os.path.join(self.model_path, "business_classifier.joblib")
            revenue_path = os.path.join(self.model_path, "revenue_estimator.joblib")
            scaler_path = os.path.join(self.model_path, "scaler.joblib")
            
            if all(os.path.exists(path) for path in [classifier_path, revenue_path, scaler_path]):
    ...
    def _save_models(self):
        ...
        try:
            joblib.dump(self.classifier, os.path.join(self.model_path, "business_classifier.joblib"))
            joblib.dump(self.revenue_estimator, os.path.join(self.model_path, "revenue_estimator.joblib"))
    ...
    def train(self, features: Optional[np.ndarray] = None, 
        ...
        if np.sum(business_mask) > 0:
            ...
            self.revenue_estimator.fit(X_train_scaled[business_mask], rev_train[business_mask])
            
            # Evaluate revenue estimator
            business_test_mask = y_test == 1
            if np.sum(business_test_mask) > 0:
                rev_pred = self.revenue_estimator.predict(X_test_scaled[business_test_mask])
    ...
    def detect_businesses(self, coordinates: List[Tuple[float, float]], 
                         features: Optional[np.ndarray] = None) -> List[Dict]:
        """
        ...
        for i, (lat, lng) in enumerate(coordinates):
            ...
            if is_business and self.revenue_estimator:
                estimated_revenue = max(0, self.revenue_estimator.predict(feature_vector_scaled)[0])
    ...
    def _classify_business_type(self, features: np.ndarray) -> str:
        ...
        """
        building_density, road_proximity, vegetation_index, population_density, economic_activity, market_proximity = features
    ...
    def cluster_businesses(self, business_data: List[Dict], eps: float = 0.01, min_samples: int = 3) -> List[Dict]:
        """
...
if __name__ == "__main__":
    ...
    for result in results:
        ...
        print(f"Business: {result['is_business']} (confidence: {result['confidence_score']:.2f})")
    ...
    if business_results:
        ...
        for cluster in clusters:
            print(f"Cluster {cluster['cluster_id']}: {cluster['business_count']} businesses")
