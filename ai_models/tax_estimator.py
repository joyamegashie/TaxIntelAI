...
class TaxEstimator:
    ...
    def _load_models(self):
        ...
        try:
            ...
            efficiency_path = os.path.join(
                self.model_path, "collection_efficiency_model.joblib"
            )
    ...
    def _save_models(self):
        ...
        try:
            joblib.dump(
                self.revenue_model,
                os.path.join(self.model_path, "revenue_model.joblib"),
            )
            joblib.dump(
                self.compliance_model,
                os.path.join(self.model_path, "compliance_model.joblib"),
            )
            joblib.dump(
                self.collection_efficiency_model,
                os.path.join(self.model_path, "collection_efficiency_model.joblib"),
            )
            joblib.dump(self.scaler, os.path.join(self.model_path, "tax_scaler.joblib"))
            joblib.dump(
                self.label_encoders,
                os.path.join(self.model_path, "label_encoders.joblib"),
            )
    ...
    def generate_synthetic_data(self, n_samples: int = 2000) -> pd.DataFrame:
        ...
        for _ in range(n_samples):
            ...
            base_revenue_per_business = (
                30000 * region_multipliers[region] * sector_multipliers[sector]
            )
            ...
            actual_tax_collected = (
                potential_tax * compliance_rate * collection_efficiency
            )
    ...
    def train(self, data: Optional[pd.DataFrame] = None):
        ...
        logger.info(
            f"Compliance Model - R²: {compliance_r2:.3f}, MAE: {compliance_mae:.3f}"
        )
        ...
        logger.info(
            f"Collection Efficiency Model - R²: {efficiency_r2:.3f}, MAE: {efficiency_mae:.3f}"
        )
        ...
        for col in ["region", "sector"]:
            if col in self.label_encoders:
                try:
                    ...
                    logger.warning(
                        f"Unseen category '{input_data[col].iloc[0]}' for {col}, using default encoding"
                    )
    ...
    def analyze_regional_opportunities(
        self, regions: List[str], sectors: List[str] = None
    ) -> List[Dict]:
        """
    ...
    def simulate_policy_impact(
        self, baseline_estimate: Dict, policy_changes: Dict
    ) -> Dict:
        """
        ...
        if "compliance_improvement" in policy_changes:
            new_estimate["compliance_rate"] = min(
                0.9,
                baseline_estimate["compliance_rate"]
                * (1 + policy_changes["compliance_improvement"]),
            )

        if "efficiency_improvement" in policy_changes:
            new_estimate["collection_efficiency"] = min(
                0.95,
                baseline_estimate["collection_efficiency"]
                * (1 + policy_changes["efficiency_improvement"]),
            )

        if "tax_rate_change" in policy_changes:
            new_estimate["tax_rate"] = baseline_estimate["tax_rate"] * (
                1 + policy_changes["tax_rate_change"]
            )

        # Recalculate tax estimates
        new_estimate["potential_tax"] = (
            new_estimate["estimated_revenue"] * new_estimate["tax_rate"]
        )
        new_estimate["expected_tax_collected"] = (
            new_estimate["potential_tax"]
            * new_estimate["compliance_rate"]
            * new_estimate["collection_efficiency"]
        )
        new_estimate["tax_gap"] = (
            new_estimate["potential_tax"] - new_estimate["expected_tax_collected"]
        )
        ...
        impact = {
            ...
            "additional_revenue": new_estimate["expected_tax_collected"]
            - baseline_estimate["expected_tax_collected"],
            "percentage_increase": (
                (
                    new_estimate["expected_tax_collected"]
                    / baseline_estimate["expected_tax_collected"]
