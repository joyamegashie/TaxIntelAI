...
async def get_geofiscal_data(
    ...
    if min_activity_score is not None:
        query = query.filter(
            GeoFiscalData.economic_activity_score >= min_activity_score
        )

    if max_activity_score is not None:
        query = query.filter(
            GeoFiscalData.economic_activity_score <= max_activity_score
        )
...
async def get_geofiscal_heatmap(
    ...
    geofiscal_data = (
        db.query(GeoFiscalData)
        .filter(
            and_(
                GeoFiscalData.latitude.between(
                    latitude - lat_range, latitude + lat_range
                ),
                GeoFiscalData.longitude.between(
    ...
    if geofiscal_data:
        avg_activity_score = sum(
            d.economic_activity_score for d in geofiscal_data
        ) / len(geofiscal_data)
        avg_business_density = sum(d.business_density for d in geofiscal_data) / len(
            geofiscal_data
        )
        avg_tax_collection = sum(d.tax_collection_rate for d in geofiscal_data) / len(
            geofiscal_data
        )
        avg_informal_economy = sum(
            d.informal_economy_percentage for d in geofiscal_data
        ) / len(geofiscal_data)

        # Find hotspots (top 20% by economic activity)
        sorted_data = sorted(
            geofiscal_data, key=lambda x: x.economic_activity_score, reverse=True
        )
    ...
    return GeoFiscalHeatmapResponse(regions=geofiscal_data, summary_stats=summary_stats)
...
async def get_regional_comparison(
    ...
    for region in region_list:
        ...
        region_data = (
            db.query(GeoFiscalData)
            .filter(GeoFiscalData.region.ilike(f"%{region}%"))
            .all()
        )

        if region_data:
            # Calculate averages for this region
            avg_activity = sum(d.economic_activity_score for d in region_data) / len(
                region_data
            )
            avg_density = sum(d.business_density for d in region_data) / len(
                region_data
            )
            avg_collection = sum(d.tax_collection_rate for d in region_data) / len(
                region_data
            )
            avg_informal = sum(
                d.informal_economy_percentage for d in region_data
            ) / len(region_data)
...
async def predict_tax_potential(
    ...
    estimated_revenue_per_km2 = (
        business_density * 50000 * economic_activity_score
    )  # $50k avg per business
    potential_tax_per_km2 = (
        estimated_revenue_per_km2
        * base_tax_rate
        * activity_multiplier
        * density_multiplier
...
async def get_geofiscal_data_by_id(
    ...
    if not geofiscal_data:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Geofiscal data not found"
        )
...
async def update_geofiscal_data(
    ...
    db_geofiscal.informal_economy_percentage = (
        geofiscal_update.informal_economy_percentage
    )
