...
async def create_tax_opportunity(
    ...
    impact_percentage = (
        (
            (opportunity.projected_collection - opportunity.current_collection)
            / opportunity.current_collection
            * 100
        )
        if opportunity.current_collection > 0
        else 0
    )
...
async def estimate_tax_potential(
    ...
    businesses = (
        ...
        .filter(InformalBusiness.region.ilike(f"%{region}%"))
        .all()
    )
    ...
    complete_records = sum(
        1 for b in businesses if b.estimated_revenue and b.tax_potential
    )
    confidence_score = (
        (complete_records / business_count) * 0.9 if business_count > 0 else 0
    )
    ...
    for business in businesses:
        ...
        breakdown_by_sector[sector]["total_tax_potential"] += (
            business.tax_potential or 0
        )
...
async def get_tax_analytics_summary(
    ...
    top_regions = (
        ...
        .group_by(TaxOpportunity.region)
        .order_by(func.sum(TaxOpportunity.potential_tax).desc())
        .limit(10)
        .all()
    )
    ...
    top_sectors = (
        ...
        .group_by(TaxOpportunity.sector)
        .order_by(func.sum(TaxOpportunity.potential_tax).desc())
        .limit(10)
        .all()
    )
...
async def get_tax_opportunity(
    ...
    opportunity = (
        db.query(TaxOpportunity).filter(TaxOpportunity.id == opportunity_id).first()
    )

    if not opportunity:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Tax opportunity not found"
        )
